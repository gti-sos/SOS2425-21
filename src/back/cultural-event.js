...
function loadBackendPRG(app) {
    ...
    app.get(`${BASE_API}/${RESOURCE}/loadInitialData`, async (req, res) => {
        db_PRG.count({}, async (err, count) => {
            if (count > 0) return res.status(409).json({ error: "Datos ya cargados." });
    ...
    app.get(`${BASE_API}/${RESOURCE}`, (req, res) => {
        ...
        if (req.query.total_event) query.total_event = parseInt(req.query.total_event);
        if (req.query.avg_ticket_price) query.avg_ticket_price = parseFloat(req.query.avg_ticket_price);
        if (req.query.total_attendance) query.total_attendance = parseInt(req.query.total_attendance);
        if (req.query.local_attendance) query.local_attendance = parseInt(req.query.local_attendance);
        if (req.query.foreign_attendance) query.foreign_attendance = parseInt(req.query.foreign_attendance);
        if (req.query.event_type) query.event_type = req.query.event_type;
        if (req.query.avg_event_duration) query.avg_event_duration = parseFloat(req.query.avg_event_duration);

        db_PRG.count(query, (err, totalCount) => {
            if (err) return res.status(500).json({ error: "Error al contar." });

            db_PRG.find(query).skip(offset).limit(limit).exec((err, docs) => {
                if (err) return res.status(500).json({ error: "Error al obtener datos." });

                const cleanDocs = docs.map(({ _id, ...rest }) => rest);
                res.status(200).json({ total: totalCount, limit, offset, data: cleanDocs });
            });
        });
    });
    ...
    app.get(`${BASE_API}/${RESOURCE}/:province/:year`, (req, res) => {
        ...
        db_PRG.findOne({ province: new RegExp(`^${province}$`, "i"), year }, (err, doc) => {
    ...
    app.post(`${BASE_API}/${RESOURCE}`, (req, res) => {
        ...
        const requiredFields = ["year", "month", "province", "total_event", "avg_ticket_price", "total_attendance", "local_attendance", "foreign_attendance", "event_type", "avg_event_duration"];
        for (let field of requiredFields) {
            if (newData[field] === undefined) return res.status(400).json({ error: `Falta el campo: ${field}` });
        }

        db_PRG.findOne({ province: new RegExp(`^${newData.province}$`, "i"), year: newData.year }, (err, existing) => {
    ...
    app.put(`${BASE_API}/${RESOURCE}/:province/:year`, (req, res) => {
        ...
        if (newData.province.toLowerCase() !== province.toLowerCase() || newData.year !== year) {
            return res.status(400).json({ error: "ID de URL y cuerpo no coinciden." });
        }

        delete newData._id;
        db_PRG.update({ province: new RegExp(`^${province}$`, "i"), year }, newData, {}, (err, numReplaced) => {
            if (numReplaced === 0) return res.status(404).json({ error: "No encontrado." });
    ...
    app.delete(`${BASE_API}/${RESOURCE}/:province/:year`, (req, res) => {
        ...
        db_PRG.remove({ province: new RegExp(`^${province}$`, "i"), year }, {}, (err, numRemoved) => {
            if (numRemoved === 0) return res.status(404).json({ error: "No encontrado." });
    ...
    app.get(`${BASE_API}/${RESOURCE}/docs`, (req, res) => {
        res.redirect("https://documenter.getpostman.com/view/XXXXXXXXXX"); // pon tu enlace aquí
    });
}
